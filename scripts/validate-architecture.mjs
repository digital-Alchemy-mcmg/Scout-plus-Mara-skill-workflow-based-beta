...
check('Unverified sample classifications expose no downstream frozen state', samples.includes("validationStatus: 'UNVERIFIED'") && !samples.includes('requires-runtime-rehash') && /precomputedFrozenSnapshot:\s*null/g.test(samples) && /precomputedArtifact:\s*null/g.test(samples), 'Fixture queries remain inspectable while unverified classification cannot masquerade as a completed MARA/artifact run.');
